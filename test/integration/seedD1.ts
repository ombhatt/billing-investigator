...
/**
 * Loads one or more generated datasets, mirroring `emitSql`: the DELETEs run
 * once, ahead of every account, so seeding a second cannot wipe the first.
 * Variadic, so `seedDataset(db, dataset)` still reads the same.
 */
