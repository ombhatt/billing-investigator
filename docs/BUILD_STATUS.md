...
| 2026-09-10 | Manual testing found a follow-up about June returning the August summary as though it answered. Follow-ups now decline out-of-scope periods by name, and every fallback states it could not answer before offering the conclusion as context. Bare month names resolve against the investigated year. 419 tests. |
| 2026-09-10 | Manual testing found a clarification reply naming two valid periods being overridden by the model's own choice. Explicitly named periods now win outright. 425 tests. |
| 2026-09-10 | Added `docs/HOW_IT_WORKS.md` — plain-language explanation of the Cloudflare components, which pipeline steps the model may touch, and the alternative stacks considered. Linked from the README top and Further reading. |
| 2026-09-10 | Unified the investigation policy across the agent loop, the deterministic runner and the pure-domain analysis. Service selection extracted to `src/domain/servicePolicy.ts`; `investigationRunner.ts` (291 lines) replaced by a wrapper over the real coordinator. Parity asserted over reprice and duplicate scenarios the golden fixture cannot show. 430 tests. |
| 2026-09-10 | Tool types preserved end to end via `src/tools/catalog.ts`. Fact reducers, input builders and step summarisers became mapped types over `ToolName`; `isAllowedTool`/`isConditionalTool` are now type predicates marking the one dynamic boundary. 437 tests. |
...
| 2026-09-13 | P1 opened. `CLAUDE.md`'s "Not in P0" section replaced with scope gates: P0 recorded closed, P1 unblocked with per-item state, P2 and the permanent exclusions kept. Added `docs/BUILD_PLAN_P1.md` — Milestones 6–10 for the duplicated-usage account, the first scenario whose honest verdict is "not correct". Corrected `BUILD_PLAN.md`'s status line, which still read "Plan only. No implementation has started." |
| 2026-09-14 | P1 Milestone 6 complete. Seed generator parameterised on an `AccountProfile`: it reads no account-specific module constant, and each profile carries its own PRNG seed, so generation order cannot affect any account's content. `emitSql` is variadic and emits the DELETEs once. `abc123` byte-identical (`2533052736db…`), golden fact block unchanged. Added a pinned-hash test for the golden account — "byte-identical across runs" compares two runs of the same code and agreed with itself throughout a mutation that moved every quantity in the file. Mutation-checked four ways. 514 tests. |
| 2026-09-14 | P1 Milestone 7 complete. Seeded `dup-7741` (Northwind Trading Co.): an ingestion replay of five days of primary-zone Workers traffic, injected before the rollup so it flows into `daily_usage`, the rated charges and the invoice. Result: reconciliation **passes** at all twelve boundaries and the bill is still overstated by $914.60 — 85% of the $1,079.60 variance. 120 probable duplicate groups, 0 exact (a repeated `event_id` is unstorable). `check_duplicate_usage` and `reconcile_invoice` proven against D1 with no agent and no model. The D1 insert caught a real defect the SQL file had hidden: both accounts shared `price-workers-2026-01`, and `emitSql` writes `INSERT OR REPLACE`, so the golden account's price rows were being silently overwritten. Per-account price ids, plus cross-account id-uniqueness tests over every id-bearing table. Also de-churned `seed/emitSql.ts`, which M6 had reformatted wholesale with a non-house-style prettier run. `abc123` byte-identical, fact block unchanged. Mutation-checked three ways. 533 tests. |
